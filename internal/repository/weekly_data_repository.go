...
func (r *WeeklyData) getTableName(tsCode string) string {
	s := model.WeeklyData{TsCode: tsCode}
	return s.TableName()
}

// SaveWeeklyData 保存周K线数据到对应的分表
func (r *WeeklyData) SaveWeeklyData(data []model.WeeklyData) error {
	if len(data) == 0 {
		return nil
	}

	// 按表名分组
	tableGroups := make(map[string][]model.WeeklyData)

	for _, item := range data {
		tableName := item.TableName()
		if tableGroups[tableName] == nil {
			tableGroups[tableName] = make([]model.WeeklyData, 0)
		}
...
// UpsertWeeklyData 更新或插入周K线数据到对应的分表（支持分批处理）
func (r *WeeklyData) UpsertWeeklyData(data []model.WeeklyData) error {
	if len(data) == 0 {
		return nil
	}
...
func (r *WeeklyData) upsertDataInBatches(tableName string, data []model.WeeklyData) error {
...
// GetWeeklyData 获取指定股票的周K线数据
func (r *WeeklyData) GetWeeklyData(tsCode string, startDate, endDate time.Time, limit int) ([]model.WeeklyData, error) {
	...
	if err := query.Find(&dataList).Error; err != nil {
		logger.Errorf("Failed to get weekly data for %s: %v", tsCode, err)
		return nil, err
	}
...
func (r *WeeklyData) GetLatestWeeklyData(tsCode string) (*model.WeeklyData, error) {
	...
	if err := r.db.Table(tableName).Where("ts_code = ?", tsCode).
		...
		logger.Errorf("Failed to get latest weekly data for %s: %v", tsCode, err)
		return nil, err
	}
...
func (r *WeeklyData) DeleteWeeklyData(tsCode string, tradeDate time.Time) error {
	...
	if err := db.Delete(&model.WeeklyData{}).Error; err != nil {
		logger.Errorf("Failed to delete weekly data %s %s: %v", tsCode, tradeDate.Format("2006-01-02"), err)
		return err
	}
...
func (r *WeeklyData) GetWeeklyDataCount(tsCode string) (int64, error) {
	if tsCode == "" {
		// 获取所有表的数据总数
		tableNames := []string{
			"weekly_data_000", "weekly_data_001", "weekly_data_002",
			"weekly_data_300", "weekly_data_301", "weekly_data_600",
			"weekly_data_601", "weekly_data_603", "weekly_data_605",
			"weekly_data_688", "weekly_data_other",
		}
...
func (r *WeeklyData) GetAllTableStats() (map[string]interface{}, error) {
	tableNames := []string{
		"weekly_data_000", "weekly_data_001", "weekly_data_002",
		"weekly_data_300", "weekly_data_301", "weekly_data_600",
		"weekly_data_601", "weekly_data_603", "weekly_data_605",
