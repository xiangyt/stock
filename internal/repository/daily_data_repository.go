...
// getTableName 根据股票代码获取对应的表名
func (r *DailyData) getTableName(tsCode string) string {
	s := model.DailyData{TsCode: tsCode}
	return s.TableName()
}

// SaveDailyData 保存日K线数据到对应的分表
func (r *DailyData) SaveDailyData(data []model.DailyData) error {
	...
	// 按表名分组
	tableGroups := make(map[string][]model.DailyData)

	for _, item := range data {
		tableName := item.TableName()
		if tableGroups[tableName] == nil {
			tableGroups[tableName] = make([]model.DailyData, 0)
		}
...
// UpsertDailyData 更新或插入日K线数据到对应的分表（支持分批处理）
...
func (r *DailyData) GetDailyDataCount(tsCode string) (int64, error) {
	if tsCode == "" {
		// 获取所有表的数据总数
		tableNames := []string{
			"daily_data_000", "daily_data_001", "daily_data_002",
			"daily_data_300", "daily_data_301", "daily_data_600",
			"daily_data_601", "daily_data_603", "daily_data_605",
			"daily_data_688", "daily_data_other",
		}
...
func (r *DailyData) GetAllTableStats() (map[string]interface{}, error) {
	tableNames := []string{
		"daily_data_000", "daily_data_001", "daily_data_002",
		"daily_data_300", "daily_data_301", "daily_data_600",
		"daily_data_601", "daily_data_603", "daily_data_605",
...
// GetAllExchangeStats 获取所有交易所的统计信息（保留兼容性）
func (r *DailyData) GetAllExchangeStats() (map[string]interface{}, error) {
	// 重新实现为基于新分表策略的统计
	return r.GetAllTableStats()
}
