...
func main() {
	...
	if err != nil {
		...
		if len(dailyData) > 0 {
			...
			fmt.Printf("  最新数据: %d 开盘:%.2f 收盘:%.2f 最高:%.2f 最低:%.2f 成交量:%d\n",
				latest.TradeDate, latest.Open, latest.Close,
	...
	if err != nil {
		...
		if len(financialData) > 0 {
			...
			fmt.Printf("  最新财务数据: %s EPS:%.4f 营业收入:%.0f 净利润:%.0f 毛利率:%.2f%%\n",
				latest.ReportDate.Format("2006-01-02"), latest.EPS,
				latest.Revenue, latest.NetProfit, latest.GrossMargin)
		}
	}
